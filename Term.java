...
public class Term implements Comparable<Term> {
    ...
    /*
        char[] temp = this.query.toCharArray();
        char[] temp1 = that.query.toCharArray();
        int sum = 0;
        int sum1 = 0;
        for (int a = 0; a < temp.length; a++) {
            sum += Character.getNumericValue(temp[a]);   
        }
        for (int a = 0; a < temp1.length; a++) {
            sum1 += Character.getNumericValue(temp1[a]);
        }
    ...
    private static class PrefixOrder implements Comparator<Term> {
        ...
        public int compare(Term v, Term w) {
            String sub1 = v.query.substring(0, Math.min(r, v.query.length()));
            String sub2 = w.query.substring(0, Math.min(r, w.query.length()));
